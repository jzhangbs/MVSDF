...
def get_surface_trace(path, epoch, sdf, resolution=100, return_mesh=False):
    ...
    if (not (np.min(z) > 0 or np.max(z) < 0)):
        ...
        meshexport.export('{0}/surface_{1}.obj'.format(path, epoch), 'obj')
...
def get_surface_high_res_mesh_simple(model, cams, resolution=100):
    # resolution=100
    grid = get_grid_uniform(resolution, cpu=True)
    z = []
    points = grid['grid_points']

    sdf = lambda x: model.implicit_network(x)[:, 0]
    get_surf = lambda x: model.implicit_network(x)[:, 1]
    # get_feat = lambda x: model.implicit_network(x)[:, 2:]
    # get_uncert = lambda p,v,f: model.uncert_network(p,v,f)[:,0]
    ...
    verts = verts + np.array([grid['xyz'][0][0], grid['xyz'][1][0], grid['xyz'][2][0]])
    # verts = (verts - 1) / 2 * size + np.expand_dims(center, axis=0)
    verts_t = torch.from_numpy(verts).float().cuda()

    surf_v = get_surf(verts_t).sigmoid()

    # grad_n = grad(verts_t).norm(dim=1)
    # # grad_n = (grad_n - grad_n.min()) / (grad_n.max() - grad_n.min() + 1e-9)
    # grad_n = (grad_n > 1).float()

    # cam_centers = - cams[:,0,:3,:3].transpose(-2,-1) @ cams[:,0,:3,3:]  # v31
    # uncert_mean = None
    # for view_i in range(cams.shape[0]):
    #     cam_center = cam_centers[view_i].cuda()
    #     vdir = F.normalize(verts_t - cam_center.view(1,3), dim=1)  # m3
    #     uncert = []
    #     for start_i in range(0, verts_t.shape[0], 50000):
    #         feat = get_feat(verts_t[start_i:start_i+50000])
    #         uncert.append(get_uncert(verts_t[start_i:start_i+50000], vdir[start_i:start_i+50000], feat))
    #     uncert = torch.cat(uncert, dim=0)
    #     uncert_mean = uncert_mean + uncert if uncert_mean is not None else uncert
    # uncert_mean /= cams.shape[0]
    # uncert_range = (0, 5)
    # uncert_vis = ( (-uncert_mean + uncert_range[1]) / (uncert_range[1]-uncert_range[0]) ).clamp(0,1).float()

    color = torch.stack([1-surf_v, surf_v, torch.zeros_like(surf_v)], dim=-1)
    # color = torch.stack([uncert_vis, surf_v, torch.zeros_like(uncert_vis)], dim=-1)

    meshexport = trimesh.Trimesh(verts, faces, normals, vertex_colors=color.detach().cpu().numpy())
...
def get_surface_high_res_mesh(sdf, resolution=100):
    ...
    # # points = grid_points
    # z = []
    # for i, pnts in enumerate(g):
    # # for i, pnts in enumerate(torch.split(points, 100000, dim=0)):
    #     z.append(sdf(pnts).detach().cpu().numpy())
    # z = np.concatenate(z, axis=0)

    z = []
    for i, pnts in enumerate(torch.split(grid_points, 50000, dim=0)):
        pnts2 = torch.bmm(vecs.unsqueeze(0).repeat(pnts.shape[0], 1, 1).transpose(1, 2), pnts.unsqueeze(-1)).squeeze() + s_mean
        z.append(sdf(pnts2).detach().cpu().numpy())
    z = np.concatenate(z, axis=0)
...
def get_grid_uniform(resolution, cpu=False):
