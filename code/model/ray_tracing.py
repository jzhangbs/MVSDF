...
class RayTracing(nn.Module):
    ...
    def sphere_tracing(self, batch_size, num_pixels, sdf, cam_loc, ray_directions, mask_intersect, sphere_intersections):
        ...
        if os.environ.get('IDR_USE_ENV', '0') == '1' and os.environ.get('IDR_RENDER', '0') == '1':
            dist_clip = 0.05
            self.sphere_tracing_iters = 40
        else:
            dist_clip = 0.5  # NOTE: hard code

        next_sdf_start = torch.zeros_like(acc_start_dis).cuda()
        next_sdf_start[unfinished_mask_start] = sdf(curr_start_points[unfinished_mask_start]).clamp(-dist_clip,dist_clip)

        next_sdf_end = torch.zeros_like(acc_end_dis).cuda()
        next_sdf_end[unfinished_mask_end] = sdf(curr_end_points[unfinished_mask_end]).clamp(-dist_clip,dist_clip)
        ...
        while True:
            ...
            while (not_projected_start.sum() > 0 or not_projected_end.sum() > 0) and not_proj_iters < self.line_step_iters:
                ...
                # Calc sdf
                next_sdf_start[not_projected_start] = sdf(curr_start_points[not_projected_start]).clamp(-dist_clip,dist_clip)
                next_sdf_end[not_projected_end] = sdf(curr_end_points[not_projected_end]).clamp(-dist_clip,dist_clip)
