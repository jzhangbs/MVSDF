...
def evaluate(**kwargs):
    ...
    model = IDRNetwork(conf=conf.get_config('model'))
    if torch.cuda.is_available():
        model.cuda()

    dataset_conf = conf.get_config('dataset')
    eval_dataset = SceneDataset(data_dir, eval_cameras, **dataset_conf)
    ...
    with torch.no_grad():
        ...
        if os.environ.get('IDR_USE_ENV', '0') == '1' and os.environ.get('IDR_RENDER', '0') == '1':
            pass
        else:
            mesh = plt.get_surface_high_res_mesh_simple(
                model=model,
                cams=eval_dataset.cams_hd,
            ...
            mesh_clean.export('{0}/surface_world_coordinates_{1}.obj'.format(evaldir, epoch), 'obj')

    if eval_rendering:
        only_cam = (os.environ.get('IDR_USE_ENV', '0') == '1' and os.environ.get('IDR_ONLY_CAM', '0') == '1')
        images_dir = '{0}/rendering'.format(evaldir)
        if only_cam: images_dir += '2'
        ...
        for data_index, (indices, model_input, ground_truth) in enumerate(eval_dataloader):
            ...
            model_input["object_mask"] = model_input["perfect_mask"].cuda()
            ...
            batch_size = 1 # ground_truth['rgb'].shape[0]
        ...
        if not only_cam:
            ...
            psnr_msg = "RENDERING EVALUATION {2}: psnr mean = {0} ; psnr std = {1}".format("%.2f" % psnrs.mean(), "%.2f" % psnrs.std(), expname)
            print(psnr_msg)
            with open(f'{evaldir}/psnr.txt', 'w') as f:
                f.write(psnr_msg+'\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--expname', type=str, default='test', help='The experiment name to be evaluated.')
    ...
    evaluate(data_dir=opt.data_dir,
             conf=opt.conf,
             expname=opt.expname,
